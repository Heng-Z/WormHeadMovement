...
ax[0].plot(t,head_curv,linewidth=2,label='original curvature')
# ax[0].plot(t[:len(denoised)],denoised,'--',linewidth=3,label='denoised')
ax[0].plot(t[:len(low_freq)],low_freq,'--',color='g',linewidth=2,label='slow dynamic mode')
ax[0].hlines([-10,0,10],xmin=0,xmax=t[-1],color='k',linestyle='--',linewidth=1)
ax[0].legend(loc='upper right',bbox_to_anchor=(1, 1.1),frameon=False,ncol=2,fontsize=10)
ax[0].set_xlim([0,t[-1]])
ax[0].set_xlabel('Time (s)')
ax[0].set_ylabel(r'Head curvature (1/L)')

ax[1].plot(t[0:len(ddnoised)],ddnoised,'C1',linewidth=2,label='denoised')
ax[1].plot(t[0:len(dlf)],dlf,'g',linewidth=2,label='slow dynamic mode')
ax[1].legend(loc='upper right',bbox_to_anchor=(1,1.1),frameon=False,ncol=2)
ax[1].set_xlim([0,t[-1]])
# ax[1].set_ylim([-18,18])
ax[1].set_xlabel('Time (s)')
ax[1].set_ylabel('Time derivative (1/(sL))')
ax[1].hlines(0, xmin=0, xmax=t[-1], color='k', linestyle='--',linewidth=1)
_ = [ax[i].tick_params(axis='both', which='major', labelsize=20) for i in range(2)]
_ = [ax[i].spines['top'].set_visible(False) for i in range(2)]
_ = [ax[i].spines['right'].set_visible(False) for i in range(2)]
_ = [ax[i].spines['bottom'].set_linewidth(2) for i in range(2)]
_ = [ax[i].spines['left'].set_linewidth(2) for i in range(2)]
for i in range(len(excursion_start_end)):
    color = 'C3' if excursion_start_end[i]['sign'] >0 else 'C4'
    ax[0].axvspan(t[excursion_start_end[i]['start']],t[excursion_start_end[i]['end']],color=color,alpha=0.3,linewidth=0)
    ax[1].axvspan(t[excursion_start_end[i]['start']],t[excursion_start_end[i]['end']],color=color,alpha=0.3,linewidth=0)
plt.tight_layout()
# plt.show()
fig.savefig('Fig2a_excursion_definition.pdf')

#%%
curvature_data = np.load('/Volumes/Lenovo/neuron_ablations/curvature/N2.npy',allow_pickle=True)
curvature = curvature_data[90,0][:,start_ind*2:]
plt.imshow(-curvature,aspect='auto',cmap='coolwarm',vmin=-15,vmax=15)
plt.colorbar()
plt.xticks([200,400,600,800,1000],[2,4,6,8,10],fontsize=14)
plt.xlabel('Time (s)',fontsize=14)
plt.ylabel('Body coordinate',fontsize=14)
plt.show()
#%%
